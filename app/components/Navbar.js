...
const Navbar = () => {
    ...
    const showNavbar = ["/", "/marketplace", "/pricing", "/learn", "/templates", "/products"].includes(pathname)

    return (
        <>
            {showNavbar && (
                <nav className='bg-white w-[80vw] text-[#2A2D3A] flex justify-between fixed top-10 right-[10vw] rounded-full p-5 px-7 z-50'>
                    ...
                    <div className='flex gap-3'>
                        <Link href={"/generate"}><button className="login bg-[#E9E9E9] text-[#010101] py-5 px-7 rounded-lg font-bold">Log in</button></Link>
                        <Link href={"/generate"}><button  className="signup bg-gray-900 text-white text-lg font-bold py-5 px-8 rounded-full">Sign up free</button></Link>
                    </div>
                </nav>
            )}
        </>
